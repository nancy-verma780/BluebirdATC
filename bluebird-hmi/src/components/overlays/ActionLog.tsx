...
import { Button, Divider, ListItem, ListItemIcon, ListItemText, ListSubheader, Typography } from "@mui/material";
...
import { Action, ColourProfile, OpacityProfile, TimedActions } from "utils/types";
...
export function isActionForCurrentSector(current_sectors: string[], action: Action): boolean {
    return (
        !action.sector ||
        action.sector.find((individual_sector) => current_sectors.includes(individual_sector)) !== undefined
    );
}

function filterTimedActions(actions: TimedActions[], current_sector: string[]): TimedActions[] {
    return actions
        .map((x) => {
            return {
                time: x.time,
                actions: x.actions.filter((x) => isActionForCurrentSector(current_sector, x)),
            } as TimedActions;
        })
...
export default function ActionLog(props: ActionLogProps) {
    ...
    return (
        ...
        >
            ...
            <ListSubheader
                style={{ ...borderStyle, width: "100%", minWidth: "300px", borderLeft: "none", borderBottom: "none" }}
            >
                Issued Actions
            </ListSubheader>
            <List
            ...
            >
                {[...filterTimedActions(actions, individualSectorIds)].reverse().map((item) =>
                        ...
                        <React.Fragment key={index}>
                            <ListItem sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                                ...
                                <div style={{ width: "100%", display: "flex", justifyContent: "space-between" }}>
                                    ...
                                    <ListItemIcon sx={{ minWidth: 0 }}>{setIcon(groupedActions)}</ListItemIcon>
                                </div>
                            </ListItem>
                            {index !== actions.length - 1 && <Divider key={`divider-${index}`} />}
                        </React.Fragment>
                    )),
                )}
                <Divider></Divider>
