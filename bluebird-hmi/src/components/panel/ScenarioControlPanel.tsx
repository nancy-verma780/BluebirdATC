...
export function ScenarioControlPanel(props: ScenarioControlPanelProps) {
    ...
    const setRadarPeriod = () => {
        const evolve_period = (document.getElementById("evolvePeriodInput") as HTMLInputElement | null).value;
        setEvolvePeriod({
    ...
    const setTickFrequency = () => {
        const tick_frequency_period = (document.getElementById("tickFrequencyPeriodInput") as HTMLInputElement | null)
            .value;
                    ...
                    {exists ? (
                        <>
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, px: 2 }}>
                                <Typography variant="body2"> {`Simulation Speed: x ${evolvePeriod / tickFrequency}`} </Typography>
