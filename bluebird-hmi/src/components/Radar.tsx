...
import { selectColourProfile, selectLineWidthProfile, selectOpacityProfile } from "utils/profiles/ProfileSelectors";
...
export default function Radar(props: RadarProps) {
    ...
    const [backgroundColour, setBackgroundColour] = useState(colourProfile.backgroundColour);
    ...
    useEffect(() => {
        ...
        setBackgroundColour(
            profileName === "presentation" && !showCoastline ? "white" : colourProfile.backgroundColour,
        );
    ...
    if (projection !== undefined && !isNaN(projection.center()[0]) && !isNaN(projection.center()[1])) {
        return (
            <>
                ...
                {colourByFL && (
                    <ColourBarGroup colourProfile={colourProfile} selectedColourScaleName={selectedColourScaleName} />
                )}
                {showActionLog && actions.length > 0 && (
                    <ActionLog colourProfile={colourProfile} opacityProfile={opacityProfile} />
                )}
        ...
        return (
            <Box sx={{ 
                ...
                <Typography variant="h5" sx={{ mt: 2, color: "white" }}>Welcome to BluebirdATC</Typography>
                <Typography variant="body1" color="white">Use the left sidebar to load a scenario.</Typography>
            </Box>            
        )
    }
}
