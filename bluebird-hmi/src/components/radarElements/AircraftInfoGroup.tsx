 ...
 //   const [selectAircraftMut] = useSelectAircraftMutation();
    ...
    useEffect(() => {
        ...
        function getAircraftInfoBlockSize(aircraft: Aircraft, infoBlockState: string) {
            ...
            let boxHeight =
                aircraft.entry_flight_level !== null && aircraft.bay !== "INCOMM" ? tallBoxHeight : middleBoxHeight;

            boxHeight = infoBlockState === InfoBlockOptions.HideGroundSpeed ? boxHeight - boxRowHeight : boxHeight;

            const boxWidth = aircraft.selected_flight_level < 100 ? narrowBoxWidth : fullBoxWidth;
        ...
        function getInfoBlockLine(aircraft: Aircraft): number[] {
            ...
            const [boxWidth, boxHeight] = getAircraftInfoBlockSize(aircraft, infoBlockState);
                ...
                aircraftInfoGroup.each(function (aircraft: Aircraft) {
                    if (aircraft.true_air_speed !== null) {
                        d3.select(this)
                            ...
                            .text((aircraft: Aircraft) => "G" + aircraft.true_air_speed.toFixed(0)); // ground speed. Using true air speed for now.
                    }
                });
            
            // For ShowBlock and HideGroundSpeed, show NFL
            if (infoBlockState === InfoBlockOptions.ShowBlock || infoBlockState === InfoBlockOptions.HideGroundSpeed) {
                aircraftInfoGroup.each(function (aircraft: Aircraft) {
                    if (aircraft.entry_flight_level !== null && aircraft.bay !== "INCOMM") {
                ...
                aircraftInfoGroup.each(function (aircraft: Aircraft) {
                    if (aircraft.exit_flight_level !== null) {
                        d3.select(this)
                            ...
                            .attr(
                                "fill",
                                getExitFlightLevelColour(aircraft, colourProfile, colourByFL, selectedColourScaleName),
                            )
                ...
                if (
                    ...
                    aircraftInfoGroup
                        ...
                        .text((aircraft: Aircraft) => Math.round(aircraft.selected_flight_level));
                }
            }
        }
        // Selection box
        ...
        aircraftInfoGroup
            ...
            .attr("width", (aircraft: Aircraft) => getAircraftInfoBlockSize(aircraft, infoBlockState)[0])
            // height depends on whether NFL is shown
            .attr("height", (aircraft: Aircraft) => getAircraftInfoBlockSize(aircraft, infoBlockState)[1])
            .attr("fill", "transparent")
            .attr("stroke", (aircraft: Aircraft) => {
                if (aircraft.callsign === selectedAircraft && colourByFL) {
                    return getAircraftColour(aircraft, colourProfile, colourByFL, selectedColourScaleName);
                } else {
                    return aircraft.callsign === selectedAircraft ? colourProfile.selectionColour : "none";
                }
            })
