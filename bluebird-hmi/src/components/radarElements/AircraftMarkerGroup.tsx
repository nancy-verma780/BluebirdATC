...
import { aircraftIconMarker, aircraftPlusMarker, aircraftStarMarker } from "assets/svgIcons/svgPaths";
   ...
   // const [selectAircraftMut] = useSelectAircraftMutation();
    ...
    const handleLeftClick = useCallback(
        (event: React.MouseEvent<SVGPathElement, MouseEvent>) => {
            if (event.button === 0) {
                const craft = d3.select(event.currentTarget as SVGPathElement).datum() as Aircraft;
                // selectAircraftMut({
    ...
    useEffect(() => {
        ...
        const aircraftIdent = aircraftMarkerGroup
            ...
            .attr("transform", (aircraft: Aircraft) => {
                const projectionData = projectedCoordinates.find((ac) => ac.callsign === aircraft.callsign);
                return `translate(${projectionData.coords}) rotate(${projectionData.heading})`;
            })
            .attr("class", (aircraft: Aircraft) => `aircraft marker-${aircraft.callsign}`)
            .attr("opacity", (aircraft: Aircraft) => {
                const colourData = aircraftColors.find((ac) => ac.callsign === aircraft.callsign);
