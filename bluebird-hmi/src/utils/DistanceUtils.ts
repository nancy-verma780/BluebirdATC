...
export function pairedHaversine(lon1: number, lat1: number, lon2: number, lat2: number): number {
    lon1 = toRadians(lon1);
    lat1 = toRadians(lat1);
    lon2 = toRadians(lon2);
    lat2 = toRadians(lat2);
...
export function moveLocation(
    ...
    let latNew = Math.asin(sinLatRad * cosNormDist + cosLatRad * sinNormDist * cosHeadingRad);
    let lonNew =
        lonRad + Math.atan2(sinHeadingRad * sinNormDist * cosLatRad, cosNormDist - sinLatRad * Math.sin(latNew));
...
export function getAircraftNextPixelPosition(
    ...
    const distanceKm = nauticalMilesToKm(aircraft.true_air_speed * vectorLengthHours);
    const futurePosition = moveLocation(aircraft.lon, aircraft.lat, aircraft.ground_track, distanceKm, earthRadiusKm);

    return getProjectedDistances(projection, aircraft.lon, aircraft.lat, futurePosition[0], futurePosition[1]);
}
...
export function getRangeRingPixelRadius(
    ...
    const rangeRingPosition = moveLocation(aircraft.lon, aircraft.lat, aircraft.heading, ringRadiusKm, earthRadiusKm);
...
export function isVerticallyClose(
    ...
    return Math.abs(aircraft.flight_level - selectedAircraftObject.flight_level) < verticalThreshold;
}

export function calculateSeparation(aircraftPair: Aircraft[]): { distanceLateralNm: number; distanceFL: number } {
    const craft1 = aircraftPair[0];
    const craft2 = aircraftPair[1];

    // Calculate lateral separation between two aircraft in Nm
    const distanceLateralKm: number = pairedHaversine(craft1.lon, craft1.lat, craft2.lon, craft2.lat);
    const distanceLateralNm: number = kmToNauticalMiles(distanceLateralKm);

    // Calculate the flight level difference
    const distanceFL: number = Math.abs(craft1.flight_level - craft2.flight_level);
