    ...
    reducers: {
        update: (state, action) => {
            ...
            state.aircraft = action.payload.aircraft.map((aircraft) => {
                ...
                return override ? { ...aircraft, lat: override.lat, lon: override.lon } : aircraft;
            });

            // temp mapping
            const callsignToAircraft = Object.fromEntries(state.aircraft.map((craft) => [craft.callsign, craft]));
        },
...
export const selectActions = (state: RootState): TimedActions[] => state.dynamicData.actions;
export const selectAircraft = (state: RootState): Aircraft[] => state.dynamicData.aircraft;
